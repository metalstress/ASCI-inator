...
class ExportWorker(QThread):
    ...
    def run(self):
        try:
            ...
            base_step_fps = max(1, int(getattr(self.main, 'base_fps', self.fps)))
            dt_export = (1.0 / base_step_fps) * speed_mul
            start_t = 0.0
...
class MainWindow(QWidget):
    ...
    def _export_compute_fps(self) -> int:
        # Match export timing to the same base step as the viewport animation.
        # Viewport advances time using base_fps (default 30) and animation_speed_percent.
        try:
            return max(1, int(getattr(self, 'base_fps', 30)))
        except Exception:
            return 30
