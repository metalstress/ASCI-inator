...
class ResponsiveRow(QWidget):
    """Row that switches between horizontal (label | stretch | widget) and vertical (label on top, widget below)
    depending on available width. Used to avoid clipping on narrow sidebars / high DPI."""

    def __init__(self, label: str, right_widget: QWidget, *,
                 label_style: str = "color: white; font-size: 16px; background: transparent;",
                 gap: int = 12,
                 stack_breakpoint: int = 360):
        super().__init__()
        self._breakpoint = stack_breakpoint
        ...
        # Right widget must not demand more width than it truly needs
        self._right.setSizePolicy(QSizePolicy.Minimum, QSizePolicy.Fixed)
        ...
        self._hwrap = QWidget()
        self._hwrap.setStyleSheet("background: transparent;")
        h = QHBoxLayout(self._hwrap)
        h.setContentsMargins(0, 0, 0, 0)
        h.setSpacing(gap)
        h.addWidget(self._label)
        h.addStretch(1)
        h.addWidget(self._right, 0, Qt.AlignRight)
        self._h = h

        # vertical layout container
        self._vwrap = QWidget()
        self._vwrap.setStyleSheet("background: transparent;")
        v = QVBoxLayout(self._vwrap)
        v.setContentsMargins(0, 0, 0, 0)
        v.setSpacing(8)
        v.addWidget(self._label)
        v.addWidget(self._right, 0, Qt.AlignLeft)
        self._v = v

        self._root.addWidget(self._hwrap)
        self._root.addWidget(self._vwrap)
        self._vwrap.hide()

    def resizeEvent(self, event):
        super().resizeEvent(event)
        w = self.width()
        stack = w < self._breakpoint
        self._hwrap.setVisible(not stack)
        self._vwrap.setVisible(stack)
...
class ScrollableTabBar(QWidget):
    ...
    def __init__(self, tab_names):
        ...
        # Контейнер для табов
        tabs_container = QWidget()
        tabs_container.setSizePolicy(QSizePolicy.Minimum, QSizePolicy.Fixed)
        tabs_layout = QHBoxLayout(tabs_container)
        tabs_layout.setContentsMargins(0, 0, 0, 0)
        ...
        # Make scroll area stretch to available width between arrows
    ...
    def selectTab(self, index):
        ...
        if 0 <= index < len(self.tab_buttons):
            ...
            # Ensure selected tab is fully visible with margin
            try:
                self.scroll_area.ensureWidgetVisible(self.tab_buttons[index], 20, 0)
            except Exception:
                pass
            # Update arrow visibility after movement
            try:
                self._updateTabArrows()
            except Exception:
                pass
    ...
    def resizeEvent(self, event):
        # Синхронизация ширины: скролл-область растягивается, стрелки показываются только при необходимости
        try:
            self._updateTabArrows()
        except Exception:
            pass
    ...
    def _updateTabArrows(self):
        # Показывать стрелки, только когда контента больше, чем вьюпорт
        if not hasattr(self, 'scroll_area'):
            return
        viewport_w = self.scroll_area.viewport().width()
        content_w = 0
        try:
            content_w = self.scroll_area.widget().sizeHint().width()
        except Exception:
            if self.scroll_area.widget() is not None:
                content_w = self.scroll_area.widget().width()
        need_arrows = content_w > max(1, viewport_w)
        if hasattr(self, 'btn_left'):
            self.btn_left.setVisible(need_arrows)
        if hasattr(self, 'btn_right'):
            self.btn_right.setVisible(need_arrows)
...
class MainWindow(QWidget):
    def __init__(self):
        ...
        self.export_duration_seconds = 6  # seconds
    ...
    def _enumerate_system_fonts(self):
        """Build a map {display_name: font_file_path} for the CANVAS renderer.

        Sources (priority order):
          1) ./canvas fonts (project folder) or bundled canvas fonts (PyInstaller)
          2) OS font directories (Windows/macOS/Linux)

        Note: We need actual file paths because the canvas uses PIL.ImageFont.truetype().
        """
        fm = {}

        # --- resolve base path (script vs PyInstaller) ---
        base_path = os.path.dirname(os.path.abspath(__file__))
        if getattr(sys, "frozen", False) and hasattr(sys, "_MEIPASS"):
            base_path = sys._MEIPASS

        # --- custom canvas fonts folder (user-managed, no system install needed) ---
        canvas_dirs = [
            os.path.join(base_path, "canvas fonts"),
            os.path.join(base_path, "canvas_fonts"),
            # also support when launched from repo root but __file__ is elsewhere
            os.path.join(os.getcwd(), "canvas fonts"),
            os.path.join(os.getcwd(), "canvas_fonts"),
        ]
        ...
        # 1) custom canvas fonts first (recursive)
        for d in canvas_dirs:
            if not os.path.isdir(d):
                continue
            for root, _, files in os.walk(d):
                for fn in files:
                    _add_font(os.path.join(root, fn))

        # 2) system fonts (recursive)
        for d in system_dirs:
            ...
            try:
                ...
                print(f"Ошибка при сканировании {d}: {e}")
    ...
    def _build_ui(self):
        ...
        preview_container.setMinimumSize(420, 260)
        ...
        try:
            ...
            pass
        root.addLayout(left_layout, 3)        # Правая часть: Табы + контент (скроллируемая панель)
        right_panel = QWidget()
        right_panel.setSizePolicy(QSizePolicy.Preferred, QSizePolicy.Expanding)
        # Сайдбар не должен схлопываться на узких/невысоких экранах
        ...
        # Контент табов: вертикальный скролл включается при нехватке места по высоте
        ...
        # Важно: добавляем правый внутренний отступ под scrollbar,
        # иначе элементы с выравниванием вправо могут визуально "резаться".
        content_holder = QWidget()
        content_holder_layout = QVBoxLayout(content_holder)
        content_holder_layout.setContentsMargins(0, 0, 14, 0)
        content_holder_layout.setSpacing(0)
        content_holder_layout.addWidget(self.tab_content)

        content_scroll.setStyleSheet("""
            QScrollArea { background: transparent; border: none; }
            QScrollBar:vertical { width: 8px; background: transparent; margin: 4px 0px 4px 0px; }
            QScrollBar::handle:vertical { background: rgba(255,255,255,0.25); border-radius: 4px; min-height: 24px; }
            QScrollBar::handle:vertical:hover { background: rgba(255,255,255,0.35); }
            QScrollBar::add-line:vertical, QScrollBar::sub-line:vertical { height: 0px; }
            QScrollBar::add-page:vertical, QScrollBar::sub-page:vertical { background: transparent; }
        """)
        content_scroll.setWidget(content_holder)
    ...
    def _create_canvas_tab(self):
        ...
        # GROUP 1: Columns + Rows (stable rows; responsive via stretch)
        # Columns
        ...
        self.stepper_cols.setFixedWidth(160)
        self.stepper_cols.setSizePolicy(QSizePolicy.Fixed, QSizePolicy.Fixed)

        cols_row = QWidget()
        cols_row.setStyleSheet("background: transparent;")
        cols_row_l = QHBoxLayout(cols_row)
        cols_row_l.setContentsMargins(0, 0, 0, 0)
        cols_row_l.setSpacing(12)
        cols_label = QLabel("columns")
        cols_label.setStyleSheet("color: white; font-size: 16px; background: transparent;")
        cols_label.setSizePolicy(QSizePolicy.Expanding, QSizePolicy.Fixed)
        cols_row_l.addWidget(cols_label)
        cols_row_l.addStretch(1)
        cols_row_l.addWidget(self.stepper_cols, 0, Qt.AlignRight)
        grid_layout.addWidget(cols_row)
        ...
        self.stepper_rows.setFixedWidth(160)
        self.stepper_rows.setSizePolicy(QSizePolicy.Fixed, QSizePolicy.Fixed)

        rows_row = QWidget()
        rows_row.setStyleSheet("background: transparent;")
        rows_row_l = QHBoxLayout(rows_row)
        rows_row_l.setContentsMargins(0, 0, 0, 0)
        rows_row_l.setSpacing(12)
        rows_label = QLabel("rows")
        rows_label.setStyleSheet("color: white; font-size: 16px; background: transparent;")
        rows_label.setSizePolicy(QSizePolicy.Expanding, QSizePolicy.Fixed)
        rows_row_l.addWidget(rows_label)
        rows_row_l.addStretch(1)
        rows_row_l.addWidget(self.stepper_rows, 0, Qt.AlignRight)
        grid_layout.addWidget(rows_row)
        ...
        freq_x_label.setFixedWidth(180)
        self.slider_freq_x = CustomSlider(0.0, 3.0, self.params.freq_x)
        try: self.slider_freq_x.setFixedHeight(36)
        ...
        freq_y_label.setFixedWidth(180)
        self.slider_freq_y = CustomSlider(0.0, 3.0, self.params.freq_y)
        try: self.slider_freq_y.setFixedHeight(36)
        except Exception: pass
        ...
        speed_x_label.setFixedWidth(180)
        self.slider_speed_x = CustomSlider(-3.0, 3.0, self.params.speed_x)
        try: self.slider_speed_x.setFixedHeight(36)
        except Exception: pass
        ...
        speed_y_label.setFixedWidth(180)
        self.slider_speed_y = CustomSlider(-3.0, 3.0, self.params.speed_y)
        try: self.slider_speed_y.setFixedHeight(36)
        except Exception: pass
        ...
        amp_label.setFixedWidth(180)
        self.slider_amp = CustomSlider(0.0, 2.0, self.params.amplitude)
        try: self.slider_amp.setFixedHeight(36)
        except Exception: pass
        ...
        contrast_label.setFixedWidth(180)
        self.slider_contrast = CustomSlider(0.5, 2.5, self.params.contrast)
        try: self.slider_contrast.setFixedHeight(36)
        except Exception: pass
        ...
        self.audio_sensitivity_display.setStyleSheet("color: white; font-size: 26px; font-weight: bold; background: transparent; padding-right: 20px;")
        ...
        for i, n in enumerate(["60","150","400","1k","2.4k","15k"]):
            row = QHBoxLayout(); row.setContentsMargins(0,8,0,8)
            lbl = QLabel(n); lbl.setStyleSheet("color: white; font-size: 16px; background: transparent;")
            lbl.setFixedWidth(180)
    ...
    def _create_color_tab(self):
        ...
        btn_random = QPushButton("random palette")
    ...
    def _create_modes_tab(self):
        ...
        # Temporarily hide 'audioreactive_alt' from UI (kept in codebase)
        self.cb_mode.addItems(["waves", "morph", "audioreactive", "contourswim"]) 
    ...
    def _create_export_tab(self):
        ...
        export_layout.addLayout(format_row)
            ...
            # duration (seconds) — replaces frames & fps
            dur_row = QHBoxLayout()
    ...
    def on_mode_changed(self, m):
        ...
        if hasattr(self, 'ar_alt_section'):
            # Hide audioreactive_alt panel while the mode is disabled in UI
            self.ar_alt_section.setVisible(False)
    ...
    def _export_compute_fps(self) -> int:
        # Use viewport timing to match what the user sees.
        # Prefer the actual preview timer interval if available.
        try:
            if hasattr(self, "timer") and self.timer is not None:
                ms = int(self.timer.interval())
                if ms > 0:
                    return max(1, int(round(1000.0 / ms)))
        except Exception:
            pass
        # Fallbacks
        try:
            return max(1, int(getattr(self, "preview_fps", 24)))
        except Exception:
            return 24
...
def on_export(self):
        ...
        worker = ExportWorker(self, fmt, fn, self._export_compute_frames(), self._export_compute_fps(), upscale, target_w, target_h, loop, crf)
        self.worker = worker
