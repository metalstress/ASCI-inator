...
class MainWindow(QWidget):
    ...
    def _create_canvas_tab(self):
        ...
        for w in [self.chk_outline, self.chk_rays, self.chk_bands, self.chk_sparkles]:
            toggles.addWidget(w)
        ...
        def add_labeled_slider(text, minv, maxv, init):
            ...
            lbl = QLabel(text); lbl.setStyleSheet("color: white; font-size: 16px; background: transparent;")
            sld = CustomSlider(minv, maxv, init)
            row.addWidget(lbl); row.addSpacing(20); row.addWidget(sld,1)
            ar_alt_layout.addLayout(row); return sld
    ...
    def render_frame_gray(self, t):
        ...
        if self.mode == "waves":
            ...
            # Static base + overlays (Stage 2: outline + rays + bands)
            try:
                from asciinator.core.edges import get_edge_data
                from asciinator.core.overlays import draw_outline, draw_rays, draw_bands, draw_sparkles, compose
                from asciinator.core.audio_input import AudioInput
                from asciinator.core.audio_analyzer import SixBandAnalyzer
                
                # Initialize audio input and analyzer (same as audioreactive)
                if not hasattr(self, '_audio_input'):
                    self._audio_input = AudioInput(samplerate=48000, blocksize=1024, channels=1)
                    self._audio_input.start()
                if not hasattr(self, '_audio_an'):
                ...
                # base grayscale used for edges/dist cache key
                g = self.base_gray
                h, w = g.shape
                
                # Convert to proper format for edge detection
                g_normalized = g.astype(np.float32) / 255.0  # Normalize to 0-1
                edges, dist = get_edge_data((h, w, 0), g_normalized.tobytes())
                
                # Get UI settings with fallbacks
                ...
                # Apply ASCII conversion with palette directly to base grayscale
                # Use fixed grid size for audioreactive_alt - much more characters
                cols, rows = 120, 80  # Fixed large grid
                cell_w_small = 6  # Fixed cell size
                cell_h_small = 8
                
                # Manually resize to grid
                from PIL import Image
                pil_gray = Image.fromarray((g * 255).astype(np.uint8)).convert('L')
                pil_resized = pil_gray.resize((cols, rows), Image.BICUBIC)
                grid = np.array(pil_resized, dtype=np.float32) / 255.0
                ascii_img = build_ascii_image_color(
                    grid, 
                    ASCII_RAMP_EXT if self.use_extended else ASCII_RAMP_PURE,
                    self.font_pil,
                    cell_w_small, 
                    cell_h_small, 
                    self.color_stops,
                    ...
                    gap_x=self.gap_x, 
                    gap_y=self.gap_y, 
                    bg_color=self.render_bg,
                    glyph_cache=self.glyph_cache
                )
                ascii_rgb = np.array(ascii_img.convert('RGB'), dtype=np.uint8)
                
                # Start with ASCII image as base
                final_rgb = ascii_rgb.copy()
                
                # Create audio-reactive overlays based on edge data
                t = time.time()
                
                # Resize edge data to match ASCII grid
                from scipy.ndimage import zoom
                edge_resized = zoom(edges, (rows/edges.shape[0], cols/edges.shape[1]), order=0)
                dist_resized = zoom(dist, (rows/dist.shape[0], cols/dist.shape[1]), order=1)
                
                # Resize edge data to match final_rgb size for effects
                edge_for_effects = zoom(edge_resized, (final_rgb.shape[0]/rows, final_rgb.shape[1]/cols), order=0)
                dist_for_effects = zoom(dist_resized, (final_rgb.shape[0]/rows, final_rgb.shape[1]/cols), order=1)
                
                if outline_enabled:
                    level = float(0.6 * bands[1] + 0.4 * bands[2])
                    if level > 0.01:  # Lower threshold
                        # Create outline effect only on edges
                        outline_mask = edge_for_effects > 0.5
                        pulse_factor = 1.0 + 3.0 * level  # Much stronger effect
                        final_rgb[outline_mask] = np.clip(final_rgb[outline_mask] * pulse_factor, 0, 255).astype(np.uint8)
                
                if rays_enabled:
                    level = float(0.4 * bands[0] + 0.4 * bands[1] + 0.2 * bands[4])
                    if level > 0.01:  # Lower threshold
                        # Simple ray effect - brighten random pixels
                        ray_mask = np.random.random((final_rgb.shape[0], final_rgb.shape[1])) < level * 0.2
                        ray_factor = 1.0 + level * 4.0  # Much stronger
                        final_rgb[ray_mask] = np.clip(final_rgb[ray_mask] * ray_factor, 0, 255).astype(np.uint8)
                
                if bands_enabled:
                    level = float(0.3 * bands[0] + 0.7 * bands[1])
                    if level > 0.01:  # Lower threshold
                        # Breathing effect on all pixels
                        breath = np.sin(t * 5.0) * level * 2.0  # Faster and stronger
                        final_rgb = np.clip(final_rgb * (1.0 + breath), 0, 255).astype(np.uint8)
                
                if sparkles_enabled:
                    level = float(0.5 * bands[4] + 0.5 * bands[5])
                    if level > 0.01:  # Lower threshold
                        # Sparkle effect
                        sparkle_mask = np.random.random((final_rgb.shape[0], final_rgb.shape[1])) < level * 0.1
                        sparkle_factor = 1.0 + level * 5.0  # Much stronger
                        final_rgb[sparkle_mask] = np.clip(final_rgb[sparkle_mask] * sparkle_factor, 0, 255).astype(np.uint8)
                
                # Store RGB for direct display, return grayscale for compatibility
                self._last_alt_rgb = final_rgb
                return g  # Return original grayscale
            except Exception as e:
    ...
    def render_frame_pil(self, t=None, font=None, cell_w=None, cell_h=None, use_cache=True, for_preview=False):
        ...
        # Handle RGB arrays from audioreactive_alt
        if len(g.shape) == 3 and g.shape[2] == 3:  # RGB array
            # Convert RGB to grayscale for ASCII processing
            g = np.mean(g, axis=2).astype(np.uint8)
        
        # Special handling for audioreactive_alt - use stored RGB
        if self.mode == "audioreactive_alt" and hasattr(self, '_last_alt_rgb'):
            # Return the RGB image directly for audioreactive_alt
            return Image.fromarray(self._last_alt_rgb, 'RGB')
