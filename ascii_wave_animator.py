...
def resize_to_char_grid(img_gray, cell_w, cell_h, cols=None, rows=None, max_cols=260, max_rows=180):
    h, w = img_gray.shape
    if cols is None:
        cols = min(max_cols, max(16, w // max(6, cell_w)))
    if rows is None:
        rows = min(max_rows, max(8, h // max(8, cell_h)))
    cols = max(8, int(cols)); rows = max(8, int(rows))
    pil = Image.fromarray((img_gray*255).astype(np.uint8)).convert('L').resize((cols, rows), Image.BICUBIC)
    grid = np.array(pil, dtype=np.float32) / 255.0
    return grid, cols, rows

def build_ascii_image_color(grid_gray, ramp, font_pil, cell_w, cell_h,
                            color_stops, invert=False, gap_x=0, gap_y=0, bg_color=(0,0,0),
                            glyph_cache=None):
    """
    Оптимизированный рендеринг ASCII с кэшированием символов
    """
    ...
    if glyph_cache is not None:
        for y in range(rows):
            ...
            for x in range(cols):
                ...
                c1 = color_stops[seg]; c2 = color_stops[seg+1]
                color = (int(c1[0] + (c2[0]-c1[0])*local_t),
                         int(c1[1] + (c2[1]-c1[1])*local_t),
                         int(c1[2] + (c2[2]-c1[2])*local_t))
                
                # Получаем глиф из кэша
                cache_key = (ch, color)
...
def apply_waves_time(base_gray, t, params: WaveParams):
    rows, cols = base_gray.shape
    yy, xx = np.mgrid[0:rows, 0:cols].astype(np.float32)
    x = xx / max(1, cols); y = yy / max(1, rows)
    A = 1.0 - base_gray
    ...
    if params.contrast != 1.0:
        mid = 0.5; out = clamp01((out - mid) * params.contrast + mid)
    return clamp01(out)
...
class FullscreenPreview(QWidget):
    ...
    def __init__(self):
        super().__init__()
        self.setWindowTitle("ASCI-инатор - Preview")
        self.label = QLabel(alignment=Qt.AlignCenter)
...
class LoaderDialog(QDialog):
    ...
    def __init__(self, title="рендер…"):
        super().__init__()
    ...
    def _spin(self):
        ...
        self.lbl.setText(f"рендер… {self.frames[self.idx]}")
    ...
    def run(self):
        try:
            ...
            for i in range(self.frames):
                if self._cancel:
                    return
                phi = (2*math.pi) * (i / (self.frames - 1)) if self.frames>1 else 0.0
...
class SettingsDialog(QDialog):
    ...
    def _build_ui(self):
        ...
        # Create tab bubbles
        tab_names = ["интерфейс", "аудио", "производительность"]
    ...
    def _create_interface_tab(self):
        ...
        # Title
        color_title = QLabel("цвет интерфейса")
    ...
    def _create_audio_tab(self):
        ...
        # Label
        device_label = QLabel("выбери устройство")
        ...
        device_combo.addItem("выбери устройство")
    ...
    def apply_settings(self):
        ...
        try:
            ...
            QMessageBox.information(
                self, 
                "настройки", 
                f"Настройки применены!\n\nШрифт: {self.font_family}\nРазмер: {self.font_size}pt"
            )
            self.accept()
...
class MainWindow(QWidget):
    ...
    def _build_ui(self):
        ...
        self.btn_second = QPushButton("второй вьюпорт")
        ...
        tab_names = ["Настройки холста", "Цвет", "Режим", "PostFX", "Экспорт"]
    ...
    def _create_canvas_tab(self):
        ...
        # === СЕКЦИЯ: Сетка ===
        grid_section, grid_layout = self._create_new_section("сетка")
        
        # ГРУППА 1: Столбцы + Строки (отступ между ними 10px)
        # Столбцы
        cols_row = QHBoxLayout()
        cols_row.setContentsMargins(0, 0, 0, 0)
        cols_label = QLabel("столбцы")
        cols_label.setStyleSheet("color: white; font-size: 16px; background: transparent;")
        self.stepper_cols = NewStepperWidget(self.grid_cols, 8, 400, self, "столбцы")
        ...
        grid_layout.addSpacing(10)  # 10px между столбцами и строками
        
        # Строки
        rows_row = QHBoxLayout()
        rows_row.setContentsMargins(0, 0, 0, 0)
        rows_label = QLabel("строки")
        rows_label.setStyleSheet("color: white; font-size: 16px; background: transparent;")
        self.stepper_rows = NewStepperWidget(self.grid_rows, 8, 240, self, "строки")
        ...
        grid_layout.addSpacing(10)  # 10px между группами
        
        # ГРУППА 2: Расстояния X и Y (отступ между ними 0px, слипшиеся)
        # Расстояние X
        gap_x_row = QHBoxLayout()
        gap_x_row.setContentsMargins(0, 12, 0, 12)  # Вертикальные отступы для компактности
        gap_x_label = QLabel("расстояние X")
        ...
        # Нет spacing - слайдеры визуально слипаются
        
        # Расстояние Y
        gap_y_row = QHBoxLayout()
        gap_y_row.setContentsMargins(0, 12, 0, 12)  # Вертикальные отступы для компактности
        gap_y_label = QLabel("расстояние Y")
        ...
        # === СЕКЦИЯ: Шрифт ===
        font_section, font_layout = self._create_new_section("шрифт")
        ...
        kegl_label = QLabel("кегль")
        kegl_label.setStyleSheet("color: white; font-size: 16px; background: transparent;")
        self.stepper_kegl = NewStepperWidget(self.anim_font_px, 8, 64, self, "кегль")
        ...
        # === СЕКЦИЯ: Символы ===
        symbols_section, symbols_layout = self._create_new_section("символы")
        
        # Чекбокс "расширенный набор"
        checkbox_row = QHBoxLayout()
        checkbox_row.setContentsMargins(0, 0, 0, 0)
        self.cb_extended = CustomCheckbox("расширенный набор")
        ...
        symbols_layout.addSpacing(10)  # Отступ перед полем ввода
        
        # Поле для кастомных символов (без надписи)
        ...
        # === СЕКЦИЯ: Волны (показывается для wave/morph/audioreactive) ===
        self.waves_section, waves_layout = self._create_new_section("волны")
        
        # ГРУППА 1: Частоты (слипшиеся)
        # Частота X
        freq_x_row = QHBoxLayout()
        freq_x_row.setContentsMargins(0, 12, 0, 12)
        freq_x_label = QLabel("частота Х")
        ...
        waves_layout.addSpacing(10)  # 10px между группами
        
        # ГРУППА 2: Скорости (слипшиеся)
        # Скорость X
        speed_x_row = QHBoxLayout()
        speed_x_row.setContentsMargins(0, 12, 0, 12)
        speed_x_label = QLabel("скорость Х")
        ...
        # ГРУППА 3: Отдельные параметры
        # Амплитуда
        amp_row = QHBoxLayout()
        amp_row.setContentsMargins(0, 12, 0, 12)
        amp_label = QLabel("амплитуда")
        ...
        # 0px между амплитудой и контрастом - слипшиеся
        ...
        # === СЕКЦИЯ: Параметры contourswim (показывается только для contourswim) ===
        self.contour_section, contour_layout = self._create_new_section("параметры contourswim")
        contour_layout.setSpacing(0)
        
        # 1. Чувствительность краев
        edge_sens_row = QHBoxLayout()
        edge_sens_row.setContentsMargins(0, 12, 0, 12)
        edge_sens_label = QLabel("чувствительность краев")
        ...
        wave_speed_label = QLabel("скорость волн")
        ...
        amplitude_label = QLabel("амплитуда колебаний")
        ...
        layers_label = QLabel("количество слоев")
        layers_label.setStyleSheet("color: white; font-size: 16px; background: transparent;")
        
        self.stepper_layers = NewStepperWidget(3, 1, 5, self, "количество слоев")
        ...
        blur_label = QLabel("размытие краев")
        ...
        glow_label = QLabel("яркость свечения")
        ...
        # По умолчанию скрываем секцию contourswim
    ...
    def _create_color_tab(self):
        ...
        # Заголовок секции
        symbols_title = QLabel("цвет символов")
        ...
        right_layout.addStretch()  # Пушаем кнопки вниз
        ...
        right_layout.addSpacing(10)  # 10px между кнопками
        ...
        btn_import_palette = QPushButton("импорт палитры")
    ...
    def _create_modes_tab(self):
        ...
        # === СЕКЦИЯ: Режим ===
        mode_section, mode_layout = self._create_new_section("режим")
        ...
        self.cb_mode.addItems(["волны", "morph", "audioreactive", "contourswim"])
        ...
        mode_layout.addSpacing(10)  # 10px между dropdown и контентом
        
        # Режим: morph
        morph_widget = QWidget()
        morph_layout = QVBoxLayout(morph_widget)
        morph_layout.setContentsMargins(0, 0, 0, 0)
        morph_layout.setSpacing(0)  # Управляем spacing вручную
        
        # Строка: скорость трансформации (дизайн как в столбцах/строках)
        morph_row = QHBoxLayout()
        morph_row.setContentsMargins(0, 0, 0, 0)  # Без отступов - контейнер уже даёт 20px
        
        morph_label = QLabel("скорость трансформации")
        morph_label.setStyleSheet("color: white; font-size: 16px; background: transparent;")
        
        # Большое число (как в столбцах)
        ...
        morph_buttons.setFixedSize(93, 48)  # Увеличиваем на 4px для border
        ...
        morph_buttons_layout.setContentsMargins(2, 2, 2, 2)  # Компенсируем border
        ...
        # Вертикальная линия-разделитель (короче на 8px сверху и снизу)
        separator_container = QWidget()
        separator_container.setFixedSize(1, 44)  # Высота = высота кнопок
        separator_container.setStyleSheet("background: transparent;")
        separator_layout = QVBoxLayout(separator_container)
        separator_layout.setContentsMargins(0, 4, 0, 4)
        separator_layout.setSpacing(0)
        ...
        morph_layout.addSpacing(10)  # 10px между элементами
        
        # Кнопка: импорт второго изображения
        morph_btn_row = QHBoxLayout()
        morph_btn_row.setContentsMargins(0, 0, 0, 0)  # Без отступов - контейнер уже даёт 20px
        
        self.btn_load_morph = QPushButton("импорт второго изображения")
        ...
        self.morph_widget = morph_widget  # Сохраняем ссылку
        
        # Режим: audioreactive
        audio_widget = QWidget()
        audio_layout = QVBoxLayout(audio_widget)
        audio_layout.setContentsMargins(0, 0, 0, 0)
        audio_layout.setSpacing(0)  # Управляем spacing вручную
        
        # Строка: чувствительность (дизайн как в столбцах/строках)
        audio_row = QHBoxLayout()
        audio_row.setContentsMargins(0, 0, 0, 0)  # Без отступов - контейнер уже даёт 20px
        
        audio_label = QLabel("чувствительность")
        ...
        audio_buttons.setFixedSize(93, 48)  # Увеличиваем на 4px для border
        ...
        audio_buttons_layout.setContentsMargins(2, 2, 2, 2)  # Компенсируем border
        ...
        audio_separator_container.setFixedSize(1, 44)  # Высота = высота кнопок
        ...
        audio_layout.addSpacing(10)  # 10px между элементами
        
        # Кнопка: открыть настройки звука
        audio_btn_row = QHBoxLayout()
        audio_btn_row.setContentsMargins(0, 0, 0, 0)  # Без отступов - контейнер уже даёт 20px
        
        self.btn_audio_settings = QPushButton("открыть настройки звука")
        ...
        self.audio_widget = audio_widget  # Сохраняем ссылку
        
        # === ВИДЖЕТ: contourswim режим ===
        ...
        contour_intensity_label = QLabel("интенсивность контуров")
        ...
        # Скрываем все режимы по умолчанию
        ...
        # Синхронизируем audio_sensitivity_display с audio_gain
    ...
    def _create_postfx_tab(self):
        ...
        layout = QVBoxLayout(widget)
        layout.setContentsMargins(0, 0, 0, 0)
        layout.setSpacing(10)  # 10px между секциями
        
        # === СЕКЦИЯ: CRT Monitor ===
        crt_section, crt_layout = self._create_new_section("crt monitor")
        
        # Строка: чекбокс "crt monitor effect" справа
        ...
        crt_layout.addSpacing(10)  # 10px между чекбоксом и слайдерами
        
        # Слайдеры CRT с отступами 10px между ними
        # Scanlines
        scanlines_row = QHBoxLayout()
        scanlines_row.setContentsMargins(0, 12, 0, 12)  # Вертикальные отступы для слайдеров
        ...
        # Строка: чекбокс "glow effect" справа
        ...
        glow_layout.addSpacing(10)  # 10px между чекбоксом и слайдерами
        ...
        # === СЕКЦИЯ: Viewport Quality ===
        quality_section, quality_layout = self._create_new_section("viewport quality")
        quality_layout.setSpacing(0)  # Управляем spacing вручную
        
        # Радио-кнопка "accurate"
        ...
        # Описание для accurate (маленький текст)
        accurate_desc = QLabel("скорость кадров при выборе accurate может снизиться до 2-3 fps,\nесли используется большое кол-во символов")
        ...
        quality_layout.addSpacing(10)  # 10px между элементами
        ...
        # Описание для fast (маленький текст)
        fast_desc = QLabel("пока что бесполезная фича")
    ...
    def _create_export_tab(self):
        ...
        export_layout.setSpacing(0)  # Управляем spacing вручную
        ...
        export_layout.addSpacing(10)  # 10px между элементами
        
        # Количество кадров (число + таблетка)
        frames_row = QHBoxLayout()
        frames_row.setContentsMargins(0, 0, 0, 0)
        frames_label = QLabel("количество кадров")
        frames_label.setStyleSheet("color: white; font-size: 16px; background: transparent;")
        self.stepper_frames = NewStepperWidget(120, 2, 600, self, "количество кадров")
        ...
        # Рассчитываем FPS из процентов: base_fps * (percent / 100)
        ...
        self.stepper_width = NewStepperWidget(512, 100, 10000, self, "ширина")
        ...
        self.stepper_height = NewStepperWidget(512, 100, 10000, self, "высота")
        ...
        # Скорость анимации (процент + слайдер)
        speed_row = QHBoxLayout()
        speed_row.setContentsMargins(0, 12, 0, 12)  # Вертикальные отступы как у слайдеров
        speed_label = QLabel("скорость анимации")
        speed_label.setStyleSheet("color: white; font-size: 16px; background: transparent;")
        
        # Процент жирным слева от слайдера
    ...
    def _create_new_section(self, title):
        ...
        if title:
            ...
            layout.addSpacing(20)  # 20px между заголовком и контентом
    ...
    def _set_animation_speed(self, percent):
        ...
        # Синхронизируем с stepper FPS в экспорте
        if self.stepper_fps:
    ...
    def on_accurate_preview_changed(self, state):
        ...
        if self.postfx.accurate_preview:
            ...
            QMessageBox.information(
                self, 
                "Accurate Preview включен",
                "Preview теперь показывает ТОЧНЫЙ результат export'а.\n\n"
                "⚠️ Это медленный режим (~10-20 FPS)\n"
                "Анимация будет обновляться каждый 2-3 кадр.\n\n"
                "Это нормально! Используйте для финальной проверки перед экспортом."
            )
        else:
    ...
    def on_mode_changed(self, m):
        ...
        # Проверка sounddevice для audioreactive
        if m == "audioreactive" and sd is None:
            QMessageBox.information(self, "микрофон", "Установите sounddevice:\npip install sounddevice")
            
        # Управление audio stream
        if self.mode == "audio" and self.running and sd is not None:
    ...
    def _start_audio_stream(self):
        ...
        try:
            ...
            QMessageBox.warning(self, "ошибка", f"Не удалось открыть микрофон:\n{e}")
            self.audio_stream = None
    ...
    def import_palette(self):
        ...
        try:
            ...
            QMessageBox.critical(self, "ошибка", f"не удалось извлечь палитру:\n{e}")
    ...
    def on_export(self):
            ...
            try:
                ...
                if not os.path.exists(ffmpeg_exe):
                    QMessageBox.warning(
                        self, 
                        "ffmpeg не найден",
                        "Для экспорта в MP4 требуется ffmpeg.\n\nУстановите: pip install imageio-ffmpeg"
                    )
                    return
            except Exception as e:
                QMessageBox.warning(
        ...
        def on_error(msg):
            ...
            error_dialog.setWindowTitle("ошибка экспорта")
    ...
    def _animate_tab_change(self, old_index, new_index):
        ...
        gap = int(width * 0.3)  # 30% gap между панелями
    ...
    def render_frame_gray(self, t):
        ...
        if self.mode == "волны":
            ...
            if self.morph_target is None or self.morph_target.shape != self.base_gray.shape:
                return apply_waves_time(self.base_gray, t, self.params)
            f = 0.5 + 0.5*math.sin(t*0.8)
            return clamp01((1-f)*self.base_gray + f*self.morph_target)
            
        elif self.mode == "audio":
            lev = self.audio_level
            amp = self.params.amplitude*(1.0 + self.audio_gain*lev)
            p = WaveParams(
                self.params.freq_x,
                self.params.freq_y,
                self.params.speed_x*(1+1.5*lev),
                self.params.speed_y*(1+1.5*lev),
                amp,
                self.params.contrast
            )
            return apply_waves_time(self.base_gray, t, p)
            
        elif self.mode == "рой":
            return self._render_particles(t)
            
        elif self.mode == "contourswim":
            return self._render_contourswim(t)
